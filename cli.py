...
def run(argv=None):
    ...
    a=sub.add_parser('init'); a.add_argument('--template', choices=['backend','frontend','devops'], default='backend'); a.add_argument('path', nargs='?', default='.')
    ...
    if args.cmd=='add-item': add_item(root,args.title,args.owner); print('Item added')
        ...
        for r in rows: print(('✅' if r[3] else '⬜')+f" [{r[0]}] {r[1]}"+(f" @{r[2]}" if r[2] else ''))
